...
def pso(func, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={},
        ...
        minstep=1e-8, minfunc=1e-8, debug=False, post_iters=None):
    """
    ...
    fitness = obj(x)
    best_fitness = post_iters(fitness, best_fitness, x, iter_num=0)
    ...
    while it <= maxiter:
        ...
        # parallelly calculate the particles' fitness
        fitness = obj(x)
        best_fitness = post_iters(fitness, best_fitness, x, iter_num=it)
            ...
            if fx < fp[i] and is_feasible(x[i, :]):
                ...
                if fx < fg:
                    ...
                    if np.abs(fg - fx) <= minfunc:
                        print('Stopping morph: Swarm best objective change less than {:}'.format(minfunc))
                        return tmp, fx
                    elif stepsize <= minstep:
                        print('Stopping morph: Swarm best position change less than {:}'.format(minstep))
                        return tmp, fx
                    else:
                        g = tmp.copy()
                        fg = fx
