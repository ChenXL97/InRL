...
class RaptorMorphGenerator(object):
    ...
    def _create_single_asset(self, root, asset_root, file, morph, width_constrain_tensor, geom_pos_constraint_tensor):
        ...
        # front_shoulder
        for start_idx, leg_root in enumerate([root[6][3][4], root[6][3][5]]):
            body = leg_root
            for i in range(4):
                idx = i
                if i != 0:
                ...
                body[1].set('size',
                            f'{width_constrain_tensor[idx, 0]:.4f}  {width_constrain_tensor[idx, 1]:.4f}  {morph[8 + idx]:.4f}')
                if i != 3:
                    body[1].set('pos', " ".join(
        ...
        actuator = root[7]
        for i in range(0, 4):
            actuator[i + 10].set('forcerange', f'{-morph[i]:.2f} {morph[i + 4]:.2f}')
            actuator[i + 14].set('forcerange', f'{-morph[i]:.2f} {morph[i + 4]:.2f}')
...
if __name__ == '__main__':
    ...
    morph_gen = RaptorMorphGenerator(num_morphs, random_morph_tensor, tmp_asset_path, morph_cfg_path, output_path)
